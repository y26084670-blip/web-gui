...
with sync_playwright() as pw:
    ...
    try:
        ...
        check('guide:no-js-reading-and-navigation', page.locator('section.chapter').count()==16 and urlsplit(page.url).fragment=='med')
        ...
        for kind in ['FMM', 'HTC']:
            ...
            link=page.locator('.general-information-guide-link')
            check(f'{kind}:help-link-target',link.get_attribute('href')=='/web-gui/user-guide/index.html' and link.get_attribute('target')=='_blank')
            check(f'{kind}:help-link-right-of-heading',link.bounding_box()['x']>page.locator('#general-information-title').bounding_box()['x'])
            page.screenshot(path=str(OUT/f'guide-link-{kind}.png'))
            page.set_viewport_size({'width':390,'height':844})
            check(f'{kind}:help-link-narrow-visible',link.is_visible() and link.bounding_box()['x']+link.bounding_box()['width']<=391)
